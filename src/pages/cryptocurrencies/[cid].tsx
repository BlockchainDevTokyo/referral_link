...
import { disconnect } from '../../redux/actions/nearAccountActions';
import { signIn } from '../../utils/NearAPI';
      ...
      <header className='flex flex-wrap pt-10'>
        <div className='flex flex-wrap container mx-auto w-full md:w-2/3 justify-between '>
          <div className='flex items-center my-5'>
            <img className='ml-5 h-11 w-auto' src={cryptocurrency.image.large} alt=''></img>
            <h1 className='text-5xl leading-none font-semibold ml-2'>
              {cryptocurrency.name}
            </h1>
            <span class='ml-2 text-sm uppercase rounded-lg bg-gray-300 px-2'>{cryptocurrency.symbol}</span>
          </div>
          <div className=''>
            <div className='text-xs'>{cryptocurrency.name} Price</div>
            <div className='text-4xl flex items-center'><span
              className='font-bold mr-2'>${cryptocurrency?.market_data?.current_price?.usd}</span>
              <span className='text-base font-bold px-2 py-1 my-2 rounded-lg flex text-white bg-red-500 items-center'>
                <Arrow className='mr-1' width='10' height='10' direction='down' color='white' />0.81%</span>
            </div>
          </div>
        </div>
        <div className='w-full flex mx-auto md:w-2/3 justify-between'>
          <div className=''>
            algorithm: <span
            className='ml-2 text-xs uppercase rounded-lg bg-gray-300 px-2'>{cryptocurrency.hashing_algorithm}</span>
          </div>
          <div className=''>
            <div className='w-full'>Country: {cryptocurrency.country_origin}</div>
            <div className='w-full'>Score: {cryptocurrency.community_score}</div>
          </div>
        </div>
        ...
        <div className='mx-5 my-5' dangerouslySetInnerHTML={{ __html: cryptocurrency.description.en }} />
      </header>
    </Main>
  );
};
