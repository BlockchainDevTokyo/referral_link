const InfoMark = ({ width, height }) => {
  return (
    <svg xmlns='http://www.w3.org/2000/svg' fill='currentColor' height={width || 16} width={height || 16}
         viewBox='0 0 24 24'>
      <path
...
const Arrow = ({ width, height, color, direction, className }) => {
  let rotate = '';
  if (direction === 'down') rotate = 'rotate-180';
  if (direction === 'right') rotate = 'rotate-90';
  if (direction === 'left') rotate = '-rotate-90';
  return (
    <svg className={rotate + ' ' + className} xmlns='http://www.w3.org/2000/svg' fill={color || 'red'}
         height={width || 16} width={height || 16} viewBox='0 0 487 487'>
